<template>
    <el-dialog
        ...
        v-if="isAuth(['ROOT', 'ORG:INSERT', 'ORG:UPDATE'])"
    ...
    methods: {
      ...
      init: function(id) {
        ...
        that.$nextTick(() => {
          ...
          that.loadDeptList();
          if (id) {
            that.$http('/medical/dept/sub/selectById', 'POST', { id: id }, true, function(resp) {
      ...
      dataFormSubmit: function() {
        ...
        this.$refs['dataForm'].validate(valid => {
          if (valid) {
            that.$http(
                `/medical/dept/sub/${!that.dataForm.id ? 'insert' : 'update'}`, 'POST',that.dataForm, true,function(resp) {
                  ElMessage({
                    message: '操作成功',
                    type: 'success'
                  });
