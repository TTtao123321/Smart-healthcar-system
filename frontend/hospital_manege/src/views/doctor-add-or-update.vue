    ...
    methods: {
      ...
      init: function(id) {
        ...
        that.$nextTick(() => {
          ...
          that.loadDeptAndSub();
          if (that.dataForm.id) {
            that.$http('/doctor/selectById', 'POST', { id: id }, true, function(resp) {
              let json = {
                '1': '在职',
                '2': '离职',
                '3': '退休'
              ...
              console.log(resp.recommended)
              that.dataForm.recommended = resp.recommended ? '推荐' : '普通';
              console.log(that.dataForm.recommended)
