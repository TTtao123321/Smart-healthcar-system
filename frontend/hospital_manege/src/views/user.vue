...
export default {
  ...
  methods: {
    loadDataList: function () {
      ...
      that.$http('user/selectUserByCondition', 'POST', data, true, function (resp) {
        ...
        for (let one of list) {
          if (one.hiredate) {
            const date = new Date(one.hiredate);
            one.hiredate = date.toLocaleDateString('en-CA'); // 'en-CA' will format as yyyy-MM-dd
          }
          if (one.status == 1) {
            one.status = '在职';
  ...
  //  JavaScript 中 Vue.js 的一个生命周期钩子函数 created。
  //  在 Vue.js 中，created 是一个组件的生命周期钩子，它在组件实例被创建之后立即调用，但在挂载之前（即页面没有渲染出来之前）就会执行。
  //  这意味着，created 钩子中的代码会在组件的数据、方法等已经初始化完成后执行，但模板尚未被渲染
  created: function () {
    this.loadDataList();
    this.loadRoleList();
    this.loadDeptList();
