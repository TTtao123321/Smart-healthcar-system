...
export default {
    ...
    methods: {
      loadDeptAndSub: function () {
        let that = this;
        that.$http('/medical/dept/selectDeptAndSub', 'GET', {}, false, function (resp) {
      ...
      loadDataList: function () {
        ...
        that.$http('/doctor/work_plan/schedule/selectDoctorScheduleByDeptSubIdAndDate', 'POST', data, true, function (resp) {
          let result = resp.result;
          that.doctors = result;
        });
      },
      ...
      deleteHandle: function (workPlanId) {
        ...
        that.$confirm('是否删除该条出诊日程?', '提示', {
          ...
          that.$http('/doctor/work_plan/schedule/deleteWorkPlan', 'POST', data, true, function (resp) {
            ElMessage({
              message: '删除成功',
              type: 'success',
              duration: 1200
