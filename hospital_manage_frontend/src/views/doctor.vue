        ...
        <el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm">
            ...
            <el-form-item>
                ...
                <el-button
                    size="medium"
                    type="primary"
                    v-if="isAuth(['ROOT', 'MEDICAL:INSERT'])"
                    @click="addHandle()"
                >
                    新增
                </el-button>
                <el-button
                    size="medium"
                    type="danger"
                    v-if="isAuth(['ROOT', 'MEDICAL:DELETE'])"
                    @click="deleteHandle()"
                >
                    批量删除
                </el-button>
...
export default {
    ...
    methods: {
      loadDataList: function() {
        ...
        that.$http('/doctor/selectConditionByPage', 'POST', data, true, function(resp) {
      ...
      expand: function(row, expandedRows) {
        ...
        if (expandedRows.length > 0) {
          ...
          console.log(data.id);
          that.$http('/doctor/selectDoctorDetailById', 'POST', data, false, function(resp) {
            that.content.id = row.id;
            console.log(resp.doctor.pid)
            that.content.photo = `${that.$minioUrl}${resp.doctor.photo}?random=${Math.random()}`
            that.content.pid = resp.doctor.pid;
            that.content.birthday = resp.doctor.birthday;
            that.content.uuid = resp.doctor.uuid;
            that.content.hiredate = resp.doctor.hiredate;
            that.content.email = resp.doctor.email;
            that.content.remark = resp.doctor.remark;
            that.content.tag = resp.doctor.tag;
            that.content.address = resp.doctor.address;
            that.content.description = resp.doctor.description;
          });
        } else {
          that.expands = [];
        }
      ...
      deleteHandle: function(id) {
        ...
        if (ids.length == 0) {
          ...
          that.$confirm('确定要删除选中的记录？', '提示', {
            ...
            that.$http('doctor/deleteDoctorByIds', 'POST', { ids: ids }, true, function(resp) {
              ElMessage({
                message: '操作成功',
                type: 'success',
                duration: 1200,
